...
public class BST {
    ...
    private Node insert(Node current, DataFormat df) {
        ...
        if(df.y < current.y) {
            ...
            current.right = insert(current.getRight(), df);
        } else {
    ...
    public void addSumAndMergeLists(){
        addSumToIndexX(root);
    }

    private void addSumToIndexX(Node node) {
        if (node == null)
            return;
    ...
    private void addSumToIndexXInner(List<DataFormat> dataArr) {
    ...
    private void updateMinMaxY(double y){
        if(this.maxY < y)
            this.maxY = y;
        else if (this.minY > y)
            this.minY = y;
